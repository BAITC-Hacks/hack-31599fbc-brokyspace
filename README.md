...
Интерфейс построен как самостоятельный финансовый intelligence-продукт: крупный hero с главным выводом, live-карточка приоритетного GID, модульная KPI-сетка и компактная навигация. В sidebar доступны три согласованные темы — `Midnight Signal`, светлая `Capital Ivory` и контрастная `Electric Market`; вместе с темой меняются фон, акценты, Plotly-графики и canvas сети. Дизайн-токены и правила компонентов описаны в [UI system](docs/UI.md).
